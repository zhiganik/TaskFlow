...
export function TaskDetailPanel({ task, columns, workspaceId, width, onResizeStart, onClose, onTaskUpdated }: TaskDetailPanelProps) {
  ...
  const isSaving = updateMutation.isPending || moveMutation.isPending
  ...
  return (
    ...
    >
          ...
          {editingTitle ? (
            ...
            <div className="group flex min-w-0 flex-1 items-start gap-1">
              ...
              <button
                ...
                className="mt-0.5 shrink-0 rounded p-0.5 text-gray-300 opacity-0 transition-opacity group-hover:opacity-100 hover:bg-gray-100 hover:text-gray-600"
              >
      ...
      <div className="flex-1 overflow-x-hidden overflow-y-auto">
        ...
        {/* due date */}
        <Section label={task.isOverdue ? '⚠ Due date (overdue)' : 'Due date'} labelRed={task.isOverdue}>
          <input
            type="date"
            value={dueDateValue}
            disabled={isSaving}
            ...
            className="block w-full rounded-md border border-gray-200 px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30 disabled:opacity-60"
          />
        </Section>
      ...
      <div className="shrink-0 border-t border-gray-100 px-4 py-3">
        ...
        {confirmDelete ? (
          <div className="space-y-2">
            <p className="text-xs text-gray-600">Delete this task? This cannot be undone.</p>
...
function Section({
  ...
  return (
    <div className="border-b border-gray-100 px-4 py-3">
      <p className={`mb-2 text-[11px] font-medium uppercase tracking-wide ${labelRed ? 'text-red-600' : 'text-gray-400'}`}>
        {label}
      </p>
      {children}
    </div>
